...
import org.andengine.grafika.MiscUtils;
import org.andengine.grafika.TextureMovieEncoder2;
import org.andengine.grafika.VideoEncoderCore;
import org.andengine.grafika.gles.EglCore;
import org.andengine.grafika.gles.FullFrameRect;
import org.andengine.grafika.gles.GlUtil;
import org.andengine.grafika.gles.Texture2dProgram;
import org.andengine.grafika.gles.WindowSurface;
 ...
 * * @since 06/01/2023
 */
class RecordFBOActivity extends BaseAppCompatActivity implements SurfaceHolder.Callback,
    ...
    private IRendererListener mIRendererListener;
...
//        sv.getHolder().addCallback(this);
     ...
     */
    protected void setRenderer(final Engine pEngine, final IRendererListener pRendererListener) {
        this.mEngine = pEngine;
        this.mIRendererListener = pRendererListener;
    }

    protected File getOutputFolder() {
        File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "ArFaces");
        if (!file.exists())
            file.mkdirs();
        return file;
    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        File outputFile = new File(getOutputFolder(), "Recording-" + System.currentTimeMillis() + ".mp4");
        outputFile.getParentFile().mkdirs();
        mRenderThread = new RenderThread(holder, new ActivityHandler(this), outputFile,
                MiscUtils.getDisplayRefreshNsec(this));
        mRenderThread.setOnMovieEncodeListener(outFile -> {
            if (outFile != null) {
                runOnUiThread(() -> Toast.makeText(getApplicationContext(), "Video is saved at: " + outFile.getPath(), Toast.LENGTH_SHORT).show());
            } else {
                runOnUiThread(() -> Toast.makeText(getApplicationContext(), "Can not record screen", Toast.LENGTH_SHORT).show());
            }
        });

        mRenderThread.setRenderer(mEngine, mIRendererListener);
...
//            mRecordRect = new Sprite2d(mRectDrawable);
        ...
        public void setRenderer(final Engine pEngine, final IRendererListener pRendererListener) {
            if (this.mConfigChooser == null) {
                ...
                if (configChooserOptions.isRequestedRGBA8888()) {
                    ...
                    mSurfaceHolder.setFormat(android.graphics.PixelFormat.RGB_565);
                }
            }
        ...
        private void surfaceCreated() {
            ...
            if (mEngineRenderer != null)
                mEngineRenderer.onSurfaceCreated(null, null);
        }
...
//            Log.d(TAG, "mRect: " + mRect);
            ...
            if (mEngineRenderer != null)
                mEngineRenderer.onSurfaceChanged(null, width, height);
        }
...
//            }
        ...
        private void startEncoder() {
            ...
            try {
                encoderCore = new VideoEncoderCore(mVideoWidth, mVideoHeight, BIT_RATE, mOutputFile);
...
//            mRecordRect.draw(mProgram, mDisplayProjectionMatrix);
            ...
            if (mEngineRenderer != null) {
                mEngineRenderer.onDrawFrame(null);
            } else {
                Log.e(TAG, "draw(): mEngineRenderer is null");
            }
