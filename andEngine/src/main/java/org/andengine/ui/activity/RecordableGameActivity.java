 ...
 * @author HungNV
 */
public abstract class RecordableGameActivity extends RecordFBOActivity implements IGameInterface, IRendererListener {
    // ===========================================================
    // Constants
    // ===========================================================
    ...
    protected void onCreate(final Bundle pSavedInstanceState) {
        ...
        getSurfaceView().setOnTouchListener(mEngine);
    }
    ...
    protected synchronized void onCreateGame() {
        ...
        final OnPopulateSceneCallback onPopulateSceneCallback = () -> {
            try {
                if (BuildConfig.DEBUG) {
                    Debug.d(RecordableGameActivity.this.getClass().getSimpleName() + ".onGameCreated" + " @(Thread: '" + Thread.currentThread().getName() + "')");
                }

                RecordableGameActivity.this.onGameCreated();
            } catch (final Throwable pThrowable) {
                Debug.e(RecordableGameActivity.this.getClass().getSimpleName() + ".onGameCreated failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
            }

            RecordableGameActivity.this.callGameResumedOnUIThread();
        };

        final OnCreateSceneCallback onCreateSceneCallback = pScene -> {
            RecordableGameActivity.this.mEngine.setScene(pScene);

            try {
                if (BuildConfig.DEBUG) {
                    Debug.d(RecordableGameActivity.this.getClass().getSimpleName() + ".onPopulateScene" + " @(Thread: '" + Thread.currentThread().getName() + "')");
                }

                RecordableGameActivity.this.onPopulateScene(pScene, onPopulateSceneCallback);
            } catch (final Throwable pThrowable) {
                Debug.e(RecordableGameActivity.this.getClass().getSimpleName() + ".onPopulateScene failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
            }
        };
        ...
        final OnCreateResourcesCallback onCreateResourcesCallback = new OnCreateResourcesCallback() {
            ...
            public void onCreateResourcesFinished() {
                try {
                    if (BuildConfig.DEBUG) {
                        Debug.d(RecordableGameActivity.this.getClass().getSimpleName() + ".onCreateScene" + " @(Thread: '" + Thread.currentThread().getName() + "')");
                    }

                    RecordableGameActivity.this.onCreateScene(onCreateSceneCallback);
                } catch (final Throwable pThrowable) {
                    Debug.e(RecordableGameActivity.this.getClass().getSimpleName() + ".onCreateScene failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
                }
            }
        };
    ...
    private void callGameResumedOnUIThread() {
        RecordableGameActivity.this.runOnUiThread(() -> {
            if (!RecordableGameActivity.this.isFinishing()) {
                RecordableGameActivity.this.onResumeGame();
            }
        });
    }
...
//        surfaceView.setOnTouchListener(mEngine);
//        this.setContentView(surfaceView, createSurfaceViewLayoutParams());
        setContentView(R.layout.activity_record_fbo);
        //Setup SurfaceView
        SurfaceView sv = findViewById(R.id.fboActivity_surfaceView);
        sv.getHolder().addCallback(this);
        sv.setOnTouchListener(mEngine);
